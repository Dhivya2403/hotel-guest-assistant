...
def check_availability(date: str):
    """Return which rooms are available on a given date (YYYY-MM-DD)."""
    available = []
    for room in ROOMS:
...
def suggest_room(guests: int):
    """Suggest the cheapest room type that fits the given number of guests."""
...
# Tool schemas Claude will use to decide when to call the above functions
TOOLS = [
    {
        "name": "check_availability",
        "description": "Check which room types are available on a specific date.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "suggest_room",
        "description": "Suggest the best/cheapest room type for a given number of guests.",
        "input_schema": {
            "type": "object",
            "properties": {
...
# ---------------------------------------------------------------------------
# 3. CHAT ENDPOINT — runs the tool-calling loop
# ---------------------------------------------------------------------------
...
def chat():
    ...
    # Loop until Claude gives a final text answer (handles tool calls in between)
    for _ in range(4):
...
# ---------------------------------------------------------------------------
# 4. FRONTEND — single page, no build step needed
# ---------------------------------------------------------------------------
